...
figure_pages_viewed_path = 'streamlit_cropping_labeling_app/fig_pages_viewed.xlsx'
final_figures_path = 'final_figures'

# Load the data from the Excel file
def load_data():
    path = 'extracted_figure_pages/extracted_figure_pages.xlsx'
...
def load_labeled_data(year):
    cropped_and_labeled_figures_path = f"cropped_and_labeled_image_data/{year}_cropped_and_labeled_figs.xlsx"
    ...
    elif 'labeled_data' not in st.session_state or st.session_state.labeled_data is None:
        if os.path.exists(cropped_and_labeled_figures_path):
            st.session_state.labeled_data = pd.read_excel(cropped_and_labeled_figures_path)
        else:
            st.session_state.labeled_data = pd.DataFrame()
...
def add_image_data(current_data, new_image_path, subcategory=''):
    labeled_data = load_labeled_data(year)
    ...
    save_labeled_data(labeled_data, year)
...
with left_column:  # Use the left column for selections and displaying the DataFrame
    ...
    year = st.selectbox('Select a Year:', range(2022, 1999, -1), index=list(range(2022, 1999, -1)).index(2007))
